...
export const toggleWishlist = async (req, res) => {
  try {
    ...
    const [medicineExists] = await pool.query(
      "SELECT id FROM medicines WHERE id = ?",
      [medicine_id],
    );

    if (!medicineExists.length) {
      return error(res, "Medicine not found. It may have been removed.", 404);
    }
    ...
    if (err.code === "ER_NO_REFERENCED_ROW_2") {
      return error(res, "Medicine not found or has been removed.", 404);
    }
