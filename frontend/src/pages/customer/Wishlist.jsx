...
const localWishlist = {
  ...
  // NEW: Clean invalid IDs
  cleanInvalid: (validIds) => {
    const validIdStrings = validIds.map((id) => id.toString());
    localStorage.setItem("wishlistIds", JSON.stringify(validIdStrings));
  },
};
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        if (isLoggedIn) {
          ...
          if (data.success) {
            const mapped = (data.data || []).map((item) => ({
              ...item,
              id: item.medicine_id, // ← YE KEY FIX HAI
            }));
            setItems(mapped);
          } else {
            setItems([]);
          ...
          if (ids.length === 0) {
            ...
            localWishlist.cleanInvalid(validIds);
        ...
        console.error(err);
        setItems([]);
        setMsg({ type: "error", text: "Failed to load wishlist items." });
      } finally {
        setLoading(false);
      }
    };
