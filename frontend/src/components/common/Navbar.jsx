...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    refreshCart();
    refreshWishlist();
  }, [location.pathname, refreshCart, refreshWishlist]);
  ...
  // Screen resize pe mobile menu band karo
  useEffect(() => {
    const handleResize = () => {
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            {!user && (
              <>
                ...
                {/* Login/Register - sirf desktop, mobile pe hamburger menu se milega */}
                <Link
                  to="/login"
                ...
                <Link
                  to="/register"
                  className="hidden md:block text-sm font-semibold text-white px-4 py-2 rounded-lg shadow-sm transition"
                  style={{ background: "#059669" }}
                >
                  Register
                </Link>
