...
export default function MedicineCard({ med }) {
  ...
  // ✅ Better stock check - handles both batch-based and direct quantity
  const inStock = (() => {
    // If explicitly marked as out of stock from backend
    if (med.stock_status === "out_of_stock") return false;

    // If expired
    if (isExpired) return false;

    // Check quantity (direct or from available_quantity)
    const qty = med.available_quantity || med.quantity || 0;
    if (qty <= 0) return false;

    // For logged-in users, also check if batch exists
    const isLoggedIn = !!localStorage.getItem("token");
    if (isLoggedIn && !med.batch_id) return false;
    ...
    if (token) {
      ...
      try {
        ...
        if (err.response?.status === 404) {
          setCartError("Medicine not found or removed");
          setTimeout(() => setCartError(null), 3000);
        } else {
          setCartError("Failed to update wishlist");
          setTimeout(() => setCartError(null), 3000);
        }
      } finally {
        setWishLoading(false);
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 hover:border-emerald-200 hover:shadow-lg transition-all duration-200 group overflow-hidden relative">
      {/* ✅ Out of Stock Overlay - More prominent */}
      {!inStock && (
        <div className="absolute inset-0 bg-gray-900/40 backdrop-blur-[2px] z-20 flex items-center justify-center pointer-events-none">
      ...
      <div className="p-3">
        <div className="flex items-start justify-between gap-1">
          ...
          <button
            onClick={toggleWishlist}
            disabled={wishLoading}
            className="flex-shrink-0 p-1 rounded-lg hover:bg-red-50 transition z-30 relative"
            title={wishlisted ? "Remove from wishlist" : "Add to wishlist"}
          >
            <span
              className={`text-base transition ${wishLoading ? "opacity-50" : ""}`}
