  ...
  if (event.event === "payment.captured" || event.event === "order.paid") {
    ...
    try {
      ...
      if (existing.length > 0) {
        console.log(
          `ℹ️ Order ${razorpay_order_id} already exists. Skipping webhook.`,
        );
      ...
      const [tempRows] = await conn.query(
        "SELECT user_id, order_data FROM temporary_orders WHERE id = ?",
        [razorpay_order_id],
      );
      if (tempRows.length === 0) {
        console.warn(
          `⚠️ Temp order ${razorpay_order_id} not found for webhook.`,
        );
      ...
      await conn.commit();
      console.log(
        `✅ Order finalized via webhook for RZP Order: ${razorpay_order_id}`,
      );
    } catch (err) {
